...
function sign( auth, request, body ) {
  ...
  if(["POST","PUT"].indexOf(request.method.toUpperCase()) !== -1 ) 
  {
    ...
    if ( request.getHeader('content-type') != 'application/x-www-form-urlencoded' ){
      ...
      shaObj.update(body);
      //request.setHeader("x-content-sha256", shaObj.getHash('B64')); // for oci
      request.setHeader("X-Hub-Signature", shaObj.getHash('B64'));
      headersToSign = headersToSign.concat([ "X-Hub-Signature" ]);
    }
  }
